...
\index{assertion}
 ...
 Assertions may be viewed as a form of documentation that is checked at run
 time: they can describe the state the code expects to find before it runs
 (its preconditions), and the state the code expects to result in when it
 is finished running (postconditions); they can also specify, and enforce,
 any invariants of a class.

 In many languages the assert condition is either true -- and the program
 continues execution -- or it is false and execution stops at that point.
 Unicon allows you to write your own custom assert procedure that can
 succeed, exit the program or fail.  If an assert routine fails then the
 calling procedure also fails. There may also be more than one assert
 condition.  The order of evaluation of the conditions is defined by the
 assert routine, which is specified by making an assignment to
 \texttt{\&assert}.

 The \texttt{\&assert} keyword is treated specially by the compiler: if it
 sees a normal invocation of \texttt{\&assert} it converts it to a user
 defined control structure and adds \texttt{\&file} and \texttt{\&line} to
 the parameters. It also surrounds the invocation with code that propagates
 any failure. A line like
   \iconcode{
   \&assert(x>0, y<10)
}
\noindent is transformed by the Unicon compiler into
\iconcode{
    (\textbackslash\&assert \& ( \&assert\{x>0, y<10, \&file, \&line\} | fail ))
}
\noindent
Here is a simple routine which, when assigned to \texttt{\&assert}, checks
each supplied parameter and depending on the value of a global variable,
either stops or fails if one of the parameters does not succeed.
...
\>\>\> if \textbackslash Abort\_on\_Failed\_Assertion then \{\\
\>\>\>\> stop(\\
\>\>\>\>\> @args[nargs+1], \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \# \&file\\
